...
#include "nxt_avr.h"
#include "twi.h"
#include "systick.h"			//fuer systick_callback()
...
static SYSTICK_VL nxt_avr_systick_vl;
...
nxt_avr_unpack(void)
{
  ...
  // calc the checksum
  // Wenn der AVR beim Empfangen eine falsche Prüfsumme empfängt, generiert dieser
  // beim Senden seinerseits eine falsche Prüfsumme, so dass die Überprüfung hier
  // nie korrekt ist.
  ...
  if (((io_from_avr->extra)&0x3ff) < (((2*3000)*1024)/14180))
  {
	  if(lowbat_cnt++>100)
		nxt_avr_power_down();
	  else if(lowbat_cnt==10)
		  TERM_STRING("\n\rLow Battery\n\r");
  }
  else
	  lowbat_cnt=0;
...
nxt_avr_init(void)
{
  ...
  systick_callback(&nxt_avr_systick_vl,nxt_avr_1kHz_update);
}

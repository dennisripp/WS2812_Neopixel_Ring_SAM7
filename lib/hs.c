...
//static uint8_t *in_buf[IN_BUF_CNT];
//static uint8_t *out_buf[OUT_BUF_CNT];
static uint8_t in_buf[IN_BUF_CNT][IN_BUF_SZ];
static uint8_t out_buf[OUT_BUF_CNT][OUT_BUF_SZ];

static uint8_t in_buf_in_ptr, out_buf_ptr;
...
int hs_enable(uint32_t baud_rate)
{
  ...
  *AT91C_US0_BRGR = ((MCK/8/baud_rate) | (((MCK/8) - ((MCK/8/baud_rate) * baud_rate)) / ((baud_rate + 4)/8)) << 16);
...
void hs_disable(void)
{
  ...
  *AT91C_PMC_PCDR = (1 << AT91C_ID_US0);
  ...
  //sp_reset(RS485_PORT); todo -> sensors1.c

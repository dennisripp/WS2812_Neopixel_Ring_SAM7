...
struct i2c_partial_transaction {
  ...
  uint8_t  stop:1;	
  uint8_t  tx:1;	
  uint8_t  last_pt:1; // Last pt in transaction
  uint16_t nbytes;	// N bytes to transfer
...
struct i2c_port_struct {
  uint32_t scl_pin;
  uint32_t sda_pin;
  uint8_t  addr_int[2]; /* Device ddress with internal address */
  uint8_t  addr;	   /* Just device address */
...
static uint8_t i2c_logb[I2C_LOG_N];
static uint32_t i2c_logn;
static void i2c_log(int port, int state, int inpin)
{
  static uint32_t last_valid;
...
i2c_timer_isr_C(void)
{
  ...
  uint32_t oer = 0;
  uint32_t odr = 0;
  uint32_t inputs = *AT91C_PIOA_PDSR;

  struct i2c_port_struct *p = i2c_port;

  uint32_t dummy = *AT91C_TC0_SR;
...
void i2c_init(void)
{
  ...
  *AT91C_TC0_RC = (MCK/2)/(4 * 9600);
  *AT91C_TC0_IER = 0x10; /* Enable RC trigger interrupt */
  *AT91C_TC0_CCR = 1; /* Enable */

  aic_mask_off  (AT91C_ID_TC0);
  aic_set_vector(AT91C_ID_TC0,                                    AIC_INT_LEVEL_NORMAL, i2c_timer_isr_C);
                            /*(AT91C_AIC_SRCTYPE_INT_HIGH_LEVEL | 4 ); */
  aic_clear     (AT91C_ID_TC0);

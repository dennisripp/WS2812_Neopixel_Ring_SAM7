...
#define OSC MCK//CLOCK_FREQUENCY
...
volatile uint8_t sound_mode = SOUND_MODE_NONE;
...
struct {
  ...
  volatile uint8_t buf_id;
  // Double buffer
/*  volatile DJ */ uint32_t buf[2][PDM_BUFFER_LENGTH];
  // Amplification LUT
  volatile uint8_t amp[257];
  // Volume val used to create the amp LUT
  volatile int32_t cur_vol;
...
//const uint8_t logvol[] = {0, 4, 16, 28, 40, 52, 64, 78, 93, 109, 128, 196, 255, 255};
//const uint8_t logvol[] = {0, 8, 24, 40, 56, 80, 104, 128, 160, 208, 255, 255};
...
void sound_init(void)
{
  ...
  *AT91C_SSC_CR = AT91C_SSC_TXEN;                                        
  ...
  aic_set_vector(AT91C_ID_SSC, AT91C_AIC_SRCTYPE_INT_POSITIVE_EDGE | AT91C_AIC_PRIOR_LOWEST ,sound_isr_C);
				           /* (AT91C_AIC_SRCTYPE_INT_POSITIVE_EDGE | 2); */
...
void sound_interrupt_enable(uint32_t typ)
{
...
void sound_fill_sample_buffer() {
  ...
  for (i = 0; i < PDM_BUFFER_LENGTH >> 3; i++) {
    uint8_t smp = (sample.count > 0 ? sample.amp[*sample.ptr] : 128);
    uint8_t msk = "\x00\x10\x22\x4a\x55\x6d\x77\x7f"[smp & 7];
    uint8_t s3 = smp >> 3;
